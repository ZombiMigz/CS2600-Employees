...
static int comparePhoneNumber(const void *targetPtr, PtrToConstEmployee tableValuePtr)
{
  return strcmp((char *)targetPtr, tableValuePtr->phone); // const void *targetPtr ==> typecast as char pointer then pass into strcmp()
}
static int compareSalary(const void *targetPtr, PtrToConstEmployee tableValuePtr)
{
  double diff = (*(double *)(targetPtr)) - (tableValuePtr->salary); // finds difference between target salary and actual salary
  return !((diff < .01) && (-1 * diff < .01));                      // return if difference is less than 1 cent
}
...
{
  return searchEmployeeTable(ptr, size, phoneNumber, comparePhoneNumber);
}
PtrToEmployee searchEmployeeBySalary(PtrToConstEmployee ptr, int size, double salary)
{
  return searchEmployeeTable(ptr, size, &salary, compareSalary);
}
